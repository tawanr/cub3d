...
/*   Updated: 2022/08/08 16:44:36 by tratanat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_wall_col(t_gamevars *gamevars, int x, t_ray *ray)
{
	...
	while (y < WHEIGHT / 2 + vert_line)
	{
		...
		if (x_offset < 0.005 || x_offset > 0.995 || \
			tex_pos < 0.01 || tex_pos > 0.99)
			color = 0;
		else
		{
			tex_pos = (int)(tex_pos * ray->texture->height) * ray->texture->width * 4;
			tex_pos += (int)((x_offset * (ray->texture->width)) * 4);
